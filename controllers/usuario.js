                ...
                bcrypt.compare(password, user.password, (err, check) => {
                    if (check) {
                        ...
                        if (params.gettoken) {
                            ...
                            var userId = user._id;
                            empresaUsuarioIds(userId).then((value) => {
                                return res.status(200).send({
                                    user,
...
/*traer las empresas del usuario*/
async function empresaUsuarioIds(user_id) {
    var empresas = await Empresa.find({ 'usuario': user_id }).select({ '_id': 1 }).exec().then(
        (empresaStored) => {
            var empresaUsuario = [];

            empresaStored.forEach((empresa) => {

                empresaUsuario.push(empresa._id);
